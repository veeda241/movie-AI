...
Python backend for a multiagent movie scene creator that uses Hugging Face text-generation models for scene planning and a routed video pipeline with a local MP4 fallback for scene playback. The Streamlit dashboard presents the output as a cinematic studio interface with scene cards, manifests, and playable media.
...
The dashboard shows a cinematic hero section, scene cards, the organizer manifest, a live Agent Processing panel, raw packet JSON, and inline MP4 previews when video files are available.

Generated scene packets, manifests, and video stubs are written to `movie_pipeline/output`.
